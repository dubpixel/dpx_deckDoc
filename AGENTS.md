...
13. **The classic UI's button bitmaps load in ASYNCHRONOUSLY over a WebSocket, well after the page itself is "loaded" — fixing gotcha #12's DOM selectors alone was NOT enough.** `waitUntil: "networkidle"` does not wait for this: a long-lived WebSocket connection (this UI sends `pages:subscribe`/`preview:page:subscribe` messages) is never counted as "in flight" the way an HTTP request is. A scrape that reads the grid right after load catches most buttons still showing one shared placeholder frame. Caught by actually auditing a completed real capture rather than trusting a clean page/button count: 3168 buttons captured with the right grid shape and no errors, but only 63 (2%) had unique image content — compare a modern-UI device's ~94% unique ratio on the same kind of check. **If you ever add a third Companion UI variant, audit its capture the same way** (image-content uniqueness across a real multi-page scrape, not just button/page counts) before trusting it — a structurally "complete" capture can still be silently wrong. Fixed with `waitForClassicGridToSettle()`: polls a cheap signature of every image's `src` until it stops changing before extracting. Verified via a synthetic Playwright fixture (`test/screenshot.classicAsyncBitmaps.test.js`) that reproduces the same async-swap behavior, since the real instance that surfaced this was no longer reachable to re-scrape (off the show network) — **the existing local `8H_FRANK_v3.0.0` capture (and its `backups/` copy) predates this fix and is still incomplete; re-scrape it next time that instance is reachable.**
